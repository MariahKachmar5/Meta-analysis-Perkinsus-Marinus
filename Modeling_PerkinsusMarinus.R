...
Merged.data$Int_ratio <- Merged.data$Mean.Intensity/100
head(Merged.data)
Merged.data$Int_ep <- pmin(pmax(Merged.data$Int_ratio, epsilon), 1 - epsilon)
head(Merged.data)
...
Regional_Intensity_Results$fdr.p.value <- p.adjust(Regional_Intensity_Results$p.value, method = "fdr")
Regional_Intensity_Results
                               
Regional_Intensity_Results$Region <- c("NANTICOKE RIVER","NANTICOKE RIVER", "NANTICOKE RIVER", "UPPER BAY", "UPPER BAY","UPPER BAY","CHESTER RIVER", "CHESTER RIVER","CHESTER RIVER","EASTERN BAY", "EASTERN BAY","EASTERN BAY",
                                        "WYE RIVER", "WYE RIVER","WYE RIVER","MILES RIVER","MILES RIVER","MILES RIVER", "BROAD CREEK","BROAD CREEK","BROAD CREEK",
                                        "HARRIS CREEK", "HARRIS CREEK","HARRIS CREEK","CHOPTANK RIVER","CHOPTANK RIVER","CHOPTANK RIVER", "LITTLE CHOPTANK RIVER","LITTLE CHOPTANK RIVER","LITTLE CHOPTANK RIVER",
                                        "PATUXENT RIVER","PATUXENT RIVER","PATUXENT RIVER", "MIDDLE BAY",  "MIDDLE BAY","MIDDLE BAY","MANOKIN RIVER","MANOKIN RIVER","MANOKIN RIVER",
                                        "POTOMAC RIVER","POTOMAC RIVER","POTOMAC RIVER", "LOWER BAY","LOWER BAY","LOWER BAY","FISHING BAY","FISHING BAY","FISHING BAY", "HONGA RIVER","HONGA RIVER","HONGA RIVER",
...
model116<- glm(Mean.Intensity~ Year, data = Nanticoke )
results116<-tidy(Anova(model116))
summary(model116)


model117<- glm(Mean.Intensity~ Year , data =UpperBay)
results117<-tidy(Anova(model117))
summary(model117)


model118<- lmer(Mean.Intensity~ Year +(1|Site), data= Chester )
results118<-tidy(Anova(model118))
summary(model118)

model119<- lmer(Mean.Intensity~ Year +(1|Site), data = EasternBay)
results119<-tidy(Anova(model119))
summary(model119)

model120<- glm(Mean.Intensity~ Year , data = Wye)
results120<-tidy(Anova(model120))
summary(model120)

model121<- lmer(Mean.Intensity~  Year +(1|Site), data = Miles)
results121<-tidy(Anova(model121))
summary(model121)

model122<- glm(Mean.Intensity~ Year , data = BroadCreek)
results122<-tidy(Anova(model122))
summary(model122)

model123<- glm(Mean.Intensity~ Year , data = HarrisCreek)
results123<-tidy(Anova(model123))
summary(model123)

model124<- lmer(Mean.Intensity~ Year +(1|Site), data = Choptank)
results124<-tidy(Anova(model124))
summary(model124)

model125<- glm(Mean.Intensity~ Year , data = LittleChoptank)
results125<-tidy(Anova(model125))
summary(model125)

model126<- glm(Mean.Intensity~ Year, data = Patuxent)
results126<-tidy(Anova(model126))
summary(model126)


model127<- lmer(Mean.Intensity~ Year+(1|Site) , data = MiddleBay)
results127<-tidy(Anova(model127))
summary(model127)

model128<- glm(Mean.Intensity~ Year , data = Manokin)
results128<-tidy(Anova(model128))
summary(model128)

model129<- glm(Mean.Intensity~ Year, data = Potomac)
results129<-tidy(Anova(model129))
summary(model129)


model130<- lmer(Mean.Intensity~ Year+(1|Site), data = LowerBay)
results130<-tidy(Anova(model130))
summary(model130)

model131<- glm(Mean.Intensity~Year , data = FishingBay)
results131<-tidy(Anova(model131))
summary(model131)


model132<- glm(Mean.Intensity~ Year , data = Honga)
results132<-tidy(Anova(model132))
summary(model132)

model133<- lmer(Mean.Intensity~ Year+(1|Site) , data = TangierS)
results133<-tidy(Anova(model133))
summary(model133)

model134<- glm(Mean.Intensity~ Year, data = HollandS)
results134<-tidy(Anova(model134))
summary(model134)

model135<- glm(Mean.Intensity~ Year , data = PocomokeS)
results135<-tidy(Anova(model135))
summary(model135)

model136<- lmer(Mean.Intensity~ Year +(1|Site), data = Rappahannock)
results136<-tidy(Anova(model136))
summary(model136)

model137<- lmer(Mean.Intensity~ Year+(1|Site), data = GWicomico)
results137<-tidy(Anova(model137))
summary(model137)

model138<- glm(Mean.Intensity~ Year , data = Corrotoman)
results138<-tidy(Anova(model138))
summary(model138)

model139<- lmer(Mean.Intensity~ Year +(1|Site) , data = Piankatank)
results139<-tidy(Anova(model139))
summary(model139)

model140<- lmer(Mean.Intensity~ Year+(1|Site), data = York)
results140<-tidy(Anova(model140))
summary(model140)

model141<- lmer(Mean.Intensity~ Year +(1|Site),data = Mobjack)
results141<-tidy(Anova(model141))
summary(model141)

model142<- lmer(Mean.Intensity~ Year +(1|Site), data = James)
results142<-tidy(Anova(model142))
summary(model142)

model143<- glm(Mean.Intensity~ Year ,data =  Stmary)
results143<-tidy(Anova(model143))
summary(model143)
## Combining Anova () chi sq stats into a table 
Regional_IntensityYear_Results <-rbind(results116, results117, results118, results119, results120, results121, results122, results123, results124, results125, results126, results127,
                                        results128, results128, results130, results131, results132, results133, results134, results135, results136, results137, results138, results139, results140,
                                        results141, results142, results143)
View(Regional_Intensity_Results)

### ADJUSTING P VALUE Intensity & Year ###

Regional_IntensityYear_Results$fdr.p.value <- p.adjust(Regional_IntensityYear_Results$p.value, method = "fdr")
Regional_IntensityYear_Results
Regional_IntensityYear_Results$Region <- c("NANTICOKE RIVER", "UPPER BAY", "CHESTER RIVER", "EASTERN BAY",
                                            "WYE RIVER", "MILES RIVER", "BROAD CREEK",
                                            "HARRIS CREEK", "CHOPTANK RIVER", "LITTLE CHOPTANK RIVER",
                                            "PATUXENT RIVER", "MIDDLE BAY",  "MANOKIN RIVER",
                                            "POTOMAC RIVER","LOWER BAY","FISHING BAY", "HONGA RIVER",
                                            ...
                                            "CORROTOMAN RIVER","PIANKATANK RIVER","YORK RIVER",
                                            "MOBJACK BAY","JAMES RIVER","ST MARY'S RIVER")
write.table(Regional_IntensityYear_Results, file="~/Documents/UMBC/GitHub/Meta-analysis-Perkinsus-Marinus/outputs/Regional_IntensityYear_Results.csv", sep=",", row.names=FALSE)
